# -*- coding: utf-8 -*-
"""
Created on Mon Apr 26 16:19:32 2022
...
def probability_sisheng(data,score,rank,province,showall=False):
    '''
    Parameters
    ----------
    data : dataframe
    ...
    if len(xian)==1 and data['batch_rank'].unique()[0]>0:#如果分数在分数线附近
        ...
        if len(batch_renshu)>0:
            ...
            result1_p1=1-norm.cdf(rank_p,loc=rank_p2022['predict_rank_p'].values,scale=rank_p2022['predict_rank_p_std'].values)#排名，算右尾
            #排名
            ...
            e=score-xian[0]
            w=np.around((math.tanh((e)/30*4-2)+1)/2,3)
            result1['probability']=(1-w)*result1_p1+w*result1_p2
...
if __name__ == '__main__':
    ...
    t=time.time()
    result=probability_sisheng(data,score=400,rank=50773,province=2,showall=False)
    a=result[['college_id','schoolname','predict_rank','minscore_rank_2020','minscore_rank_2019_wen',\
